...
This folder re-implements `../PoEformance.java` in .NET, reading
`PathOfExile.exe` directly — no Ghidra required. It ships two front-ends over a
shared core (`RecoveryRunner`): a **CLI** (`PoEformance`) and a
**WPF GUI** (`PoEformance.Gui`).
...
| Project | What it is |
|---|---|
| `PoEformance` | Core logic + CLI (`RecoveryRunner` does the work) |
| `gui/PoEformance.Gui` | WPF desktop front-end (`net10.0-windows`, references the core) |
| `tests/PoEformance.Tests` | xUnit suite |

`PoEformance.sln` ties all three together.

## Why this works without Ghidra

The Ghidra script relied on four services. Here is what replaces each:

| Ghidra service | PoEformance replacement | File |
|---|---|---|
...
```bash
cd PoEformance
dotnet build PoEformance.sln -c Release   # build core + GUI + tests
```
...
```bash
dotnet run -c Release --project gui/PoEformance.Gui
```
...
```bash
dotnet run -c Release --project PoEformance -- "C:\path\to\PathOfExile.exe" --verbose
```
...
Produce a single self-contained `PoEformance.Gui.exe` (no .NET install
needed to run it):

```powershell
./publish-gui.ps1
# -> gui/PoEformance.Gui/bin/Release/net10.0-windows/win-x64/publish/PoEformance.Gui.exe
```
...
A small xUnit suite under `tests/PoEformance.Tests` exercises the pipeline
...
```bash
cd PoEformance
dotnet test tests/PoEformance.Tests
```
...
| PoEformance | Ghidra script (`PoEformance.java`) |
|---|---|
